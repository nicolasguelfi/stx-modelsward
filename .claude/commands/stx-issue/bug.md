Create a GitHub **bug report** with auto-collected environment metadata.
...
Parse `$ARGUMENTS`:
- All tokens = **description** (free text describing the bug)
- `--help` → show the help section below and stop

If description is missing, ask the user to describe the bug.
...
- `READ` → skip labels, use title prefix `[Bug]`
...
```bash
gh issue create --repo <owner>/<repo> --title "[Bug] <title>" --body "<body>"
```
...
```
/stx-issue:bug — Report a bug with auto-collected metadata

Usage:
  /stx-issue:bug <description>

Examples:
  /stx-issue:bug st_grid does not render when cols="1fr 2fr" on mobile
  /stx-issue:bug st_book pagination breaks with 50+ blocks
  /stx-issue:bug AI image generation fails with Google Imagen provider
