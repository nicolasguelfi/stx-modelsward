Create a GitHub **documentation improvement** issue with auto-collected environment metadata.
...
Parse `$ARGUMENTS`:
- All tokens = **description** (free text describing the documentation improvement)
- `--help` → show the help section below and stop

If description is missing, ask the user to describe the improvement.
...
3. Documentation issues are typically routed to `streamtex-docs`, but:
   - Documentation about library API → `streamtex` (README, docstrings)
   - Documentation about Claude profiles → `streamtex-claude`
...
- `READ` → skip labels, use title prefix `[Docs]`
- No access → stop with error message

### Step 5 — Gather additional details

The description is sufficient for documentation improvements. No additional details needed.
...
## Current State
<what the documentation currently says or is missing>
...
**With write access:**
```bash
gh issue create --repo <owner>/<repo> --title "<title>" --body "<body>" --label "documentation"
```

**Without write access (read-only):**
```bash
gh issue create --repo <owner>/<repo> --title "[Docs] <title>" --body "<body>"
```
...
```
/stx-issue:docs — Request a documentation improvement with auto-collected metadata

Usage:
  /stx-issue:docs <description>

Examples:
  /stx-issue:docs Add example for st_overlay positioning
  /stx-issue:docs Missing API reference for st_mermaid options
  /stx-issue:docs Clarify st_book pagination modes in the intro manual
