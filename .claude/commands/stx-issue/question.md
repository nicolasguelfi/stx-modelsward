Create a GitHub **question** with auto-collected environment metadata.
...
Parse `$ARGUMENTS`:
- All tokens = **description** (free text describing the question)
- `--help` → show the help section below and stop

If description is missing, ask the user to describe their question.
...
3. If NOT in a StreamTeX repo (user project), route intelligently:
   - Question about library API, `st_*` functions → `streamtex`
   - Question about documentation, manuals → `streamtex-docs`
   - Question about Claude profiles, commands → `streamtex-claude`
...
- `READ` → skip labels, use title prefix `[Question]`
- No access → stop with error message

### Step 5 — Gather additional details

The description is sufficient for questions. No additional details needed.
...
## Context
<any relevant context from the environment>
...
**With write access:**
```bash
gh issue create --repo <owner>/<repo> --title "<title>" --body "<body>" --label "question"
```

**Without write access (read-only):**
```bash
gh issue create --repo <owner>/<repo> --title "[Question] <title>" --body "<body>"
```
...
```
/stx-issue:question — Ask a question with auto-collected metadata

Usage:
  /stx-issue:question <description>

Examples:
  /stx-issue:question How to use st_collection with custom routes?
  /stx-issue:question What is the recommended way to handle state in blocks?
  /stx-issue:question Can st_grid support nested grids?
